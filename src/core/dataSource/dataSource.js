...
import { assign, defer, deepclone, pipe, stableStringifyObject } from "../utils";
...
export function dataSource(config) {
    return observable(
        dataSource.nonObservable(createConfig(config)), 
        dataSource.decorate
    );
}

dataSource.nonObservable = function (config) {
    ...
    const functions = {
        ...
        get availability() {
            ...
            return this.availabilityPromise.case({
                fulfilled: v => v,
                pending: () => { debugger; console.warn('Requesting availability before availability loaded. Will return empty. Recommended to await promise.'); console.trace(); return empty },
        ...
        */
        ...
        enqueue(query) {
            return new Promise((resolve, reject) => {
                ...
                defer(() => this.processQueue(this.queue, reader));
            })
        },
        processQueue(queue, reader) {
            return pipe(
                this.resolveCached.bind(this),
                this.combineQueries.bind(this), 
                this.sendQueries(reader).bind(this),
        ...
        sendQueries(reader) {
            return function (queries) {
                return queries.map(({ query, resolves, rejects }) => {
                    ...
                    const promise = reader.read(query);
                    return {
