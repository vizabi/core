...
import { action, reaction, trace, observable } from 'mobx'
import { FULFILLED } from 'mobx-utils'
import { assign, relativeComplement, configValue, parseConfigValue, ucFirst, stepGeneratorFunction, inclusiveRange } from '../utils';
...
frame.nonObservable = function(config, parent) {
    ...
    const functions = {
        ...
        get stepScale() {
            ...
            return d3ScaleLinear(d3Range(0, this.stepCount), frameValues); 
        },
        get stepCount() {
        ...
        get playbackSteps() { return this.config.playbackSteps },
        playing: false,
        togglePlaying() {
            this.playing ?
                this.stopPlaying() :
    ...
    const enc = encoding.nonObservable(config, parent);

    return assign(enc, functions);
}