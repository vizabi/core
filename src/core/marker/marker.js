...
import { assign, isProperSubset, combineStates, isString, isNonNullObject } from "../utils";
...
export function marker(config) {
    return observable(marker.nonObservable(createConfig(config)), {
        config: observable.ref
    });
}

marker.nonObservable = function(config) {
    ...
    let functions = {
        ...
        get requiredEncodings() { return this.config.requiredEncodings },
        ...
        get dataMapCache() {
            ...
            for (let [name, encoding] of Object.entries(this.encoding)) {
                ...
                else if (isProperSubset(encoding.data.space, this.data.space) 
                    || !this.isRequired(name)
                    || encoding.data.config.modelType == 'entityPropertyDataConfig')
        ...
        // TODO possibly remove this and instead just set frame to value
        ...
        destruct() {
            this.data.destruct();
            for (const enc in this.encoding) {
                enc.destruct();
            }
        }
